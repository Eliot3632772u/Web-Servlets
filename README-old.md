...
1️⃣ The Problem Before Servlet 3.0

Before Servlet 3.0:

Servlets could NOT parse multipart/form-data

request.getParameter() did NOT work for file uploads

Developers had to use external libraries like:

Apache Commons FileUpload

Why?

Because when a browser uploads a file, it does NOT send:
...
Normal servlet parsing could not understand this format.

So developers had to manually parse raw input streams 😵‍💫

2️⃣ What Servlet 3.0 Solved

Servlet 3.0 introduced:

✔ Built-in multipart request parsing
...
👉 Any Servlet 3.0+ container (Tomcat 7+) handles multipart parsing automatically.

No external library needed.

3️⃣ How multipart/form-data Works

When this HTML form is submitted:
...
So the request becomes a collection of parts.

Servlet 3.0 automatically parses them.

4️⃣ What @MultipartConfig Actually Does
@MultipartConfig
@WebServlet("/upload")
...
👉 "This servlet expects multipart/form-data requests."

If you don’t add this annotation:
...
Because the container refuses to parse multipart content.

5️⃣ Understanding @MultipartConfig Attributes
...
Used when file exceeds memory threshold

Important:

⚠ It is NOT automatic permanent storage.
...
Files smaller than 1MB → stored in memory

Files larger than 1MB → stored temporarily on disk

Default = 0
Which means: always write to disk immediately.
...
Maximum total size of entire multipart request.

If multiple files exceed this → exception thrown.

6️⃣ What Is a Part?

javax.servlet.http.Part

Represents ONE section of a multipart request.
...
Important Part Methods
part.getName();               // form field name
part.getSize();               // file size in bytes
part.getContentType();        // mime type
part.getSubmittedFileName();  // original filename
part.write("file.png");       // save file
part.delete();                // delete temp file
part.getInputStream();        // raw content
part.getHeader("header-name");
7️⃣ How Upload Works Internally

Let’s go step-by-step:

User uploads file →
Tomcat parses multipart →
Creates Part objects →
Stores file temporarily (memory or disk) →
Makes them available via request.getPart() →
You save permanently.

8️⃣ Example 1: Basic File Upload
HTML
...
public class UploadServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        filePart.write("/tmp/uploads/" + fileName);

        resp.getWriter().println("File uploaded!");
    }
}

Done. No external libraries.

9️⃣ Example 2: Multiple Files
HTML
...
for (Part part : parts) {
    ...
    if (part.getName().equals("images")
        ...
        part.write("/tmp/uploads/" + fileName);
    }
}
🔟 Example 3: Text + File Together
<input type="text" name="username">
<input type="file" name="avatar">

In servlet:

String username = request.getParameter("username"); // works
